	...
	connection.query(
		...
		(err, rows) => {
			if (rows) {
				console.log();
				itens.forEach((item) => {
					connection.query(`INSERT INTO item_pedido (id_pedido, id_produto, quantidade, nome_disco, artista, valor) values (
						${rows[0].id_pedido}, ${item.id_produto}, ${item.qtd}, "${item.nome_disco}", "${item.artista}", ${item.valor});`);
				});
			}
		}
	);
...
app.post("/pedido", (req, res) => {
	...
	connection.query(
		`
	SELECT * FROM PEDIDO WHERE NF = "${req.body.NF}"`,
		(err, rows) => {
			if (rows) {
				res.send(rows[0]);
			}
...
app.post("/pedido/itens", (req, res) => {
	connection.query(
		`SELECT * FROM ITEM_PEDIDO WHERE id_pedido = ${req.body.id_pedido}`,
		(err, rows) => {
			if (rows) {
				res.send(rows);
			}
...
app.post("/endereco", (req, res) => {
	...
	connection.query(
		`
	SELECT * FROM ENDERECO_ENTREGA WHERE ID_ENDERECO = ${req.body.id_endereco}`,
		(err, rows) => {
			if (rows) {
				res.send(rows[0]);
			}
...
app.put("/pedido/alterar", (req, res) => {
	console.log(req.body);
	connection.query(
		`
	  UPDATE pedido SET status = "${req.body.status}" WHERE id_pedido = ${req.body.id_pedido}`,
		(err, result) => {
			if (result.affectedRows > 0) {
				res.send("Status Alterado com Sucesso");
			} else {
				res.send("Status Não Alterado");
			}
		}
	);
});
